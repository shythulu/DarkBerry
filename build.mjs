...
for (const [port, text] of [["kitty", kittyT], ["ghostty", ghosttyT], ["obsidian", obsidianT]])
...
for (const port of ["kitty", "ghostty", "firefox", "vscode", "obsidian"]) for (const o of readJson(`src/overrides/${port}.json`).overrides || []) traceExpr(port, `${o.key} (override)`, o.value);
...
for (const k of order) {
  const u = usage[k];
  usageMd += `| \`${k}\` | ${u ? [...u.roles].filter((r) => !r.startsWith("ansi")).map((r) => `\`${r}\``).join(", ") || "direct only" : "unused"} | ${["kitty", "ghostty", "vscode", "firefox", "obsidian"].map((p) => u?.ports[p] || "").join(" | ")} |\n`;
}
